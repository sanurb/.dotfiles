...
## Dotfiles integration — read first

Read [LOCAL.md](LOCAL.md) before the upstream guidance below. This setup uses
the persistent `computer` MCP server for GUI workflows. LOCAL.md documents the
repo-owned Pi configuration and overrides upstream transport defaults. Installed
tool schemas take precedence over stale examples.
