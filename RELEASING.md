...
SBOM, a Homebrew cask update, and a GitHub Release.
...
The Homebrew cask and the GitHub Release archives carry only the
...
2. **Homebrew tap repo:** `github.com/sanurb/homebrew-tap`. Create it as an
   empty public repo — GoReleaser populates `Casks/dots.rb` on each
   release.
3. **Homebrew tap layout:** `Casks/dots.rb` is generated by GoReleaser.
   The tap's root `tap_migrations.json` maps the legacy `dots` formula to
   the cask so existing installations upgrade cleanly.
4. **Repo secret `HOMEBREW_TAP_TOKEN`** on `sanurb/.dotfiles`. A
...
- A Homebrew cask commit on `sanurb/homebrew-tap`
...
- [ ] If anything in the install script changed, the Homebrew cask change,
...
2. **Delete the tag** locally and on the remote:
   ...
   ```
3. **Revert the Homebrew cask bump** on `sanurb/homebrew-tap`:
   ```sh
