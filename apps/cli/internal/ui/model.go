...
func (m Model) View() string {
	...
	case stepRealizing:
		return styPanel.Render(strings.Join([]string{
			...
			m.progress.View(),
			"",
			styMuted.Render("moon run dotfiles:deploy"),
		}, "\n"))
	case stepDone:
...
func (m *Model) welcomeForm() *huh.Form {
	...
	body := strings.Join([]string{
		...
		"  • run moon run dotfiles:deploy (gated by cli:check)",
	}, "\n")
	if m.mode == ModeSync {
		title = "dots sync"
		body = strings.Join([]string{
...
func (m Model) renderDone() string {
	...
	lines = append(lines,
		...
		fmt.Sprintf("  %s deploy  %s %s",
			styBadgeOK,
			styMuted.Render("→"),
			styBody.Render(fmt.Sprintf("moon run dotfiles:deploy in %.1fs", float64(m.realizeResult.DurationMs)/1000.0))),
		"",
